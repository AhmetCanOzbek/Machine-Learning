function code = genFunctionCell(net,name,filepath)
  import nnet.codegen.*;
  code = [...
...
function code = syntaxHelp(net,name)
  ...
  code{end+1} = '';
  code{end+1} = ['  X = ',num2str(net.numInputs),'xTS cell, ' num2str(net.numInputs) ' inputs over TS timsteps'];
  ...
  if (numFcns > 0) && (net.numInputDelays > 0)
    ...
    for j=1:numFcns
      ...
      for k=1:numel(fields)
        fields{k} = inputSettingName(i,j,fields{k});
      end
      fields = sort(fields);
      code{end+1} = [nextVar ' = ' module '_apply(' commaList([{lastVar},fields]) ');'];
      lastVar = 'temp';
    end
    code = [...
  ...
  if numFcns == 0
    ...
    if (numFcns == 0)
      ...
      for j=1:numFcns
        ...
        fields = sort(fields);
        code{end+1} = [resultvar ' = ' module '_apply(' commaList([{inputvar},fields]) ');'];
        inputvar = resultvar;
      end
    end
  end
...
function code = simulateOutput(net,i)
  ...
  if (numFcns == 0)
    ...
    for j=numFcns:-1:1
      ...
      for k=1:numel(fields)
        fields{k} = outputSettingName(i,j,fields{k});
      end
      fields = sort(fields);
      code{end+1} = [resultvar ' = ' module '_reverse(' commaList([{inputvar},fields]) ');'];
      inputvar = resultvar;
    end
  end
end
