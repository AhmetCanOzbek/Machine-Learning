...
% $Revision: 1.1.6.22 $ $Date: 2013/05/13 23:10:12 $
...
function net = formatNet(net)
  if isempty(net.performFcn)
    warning('nnet:trainbfg:Performance',nnwarning.empty_performfcn_corrected);
    net.performFcn = 'mse';
  end
end
...
function [calcNet,tr] = train_network(archNet,rawData,calcLib,calcNet,tr)
  ...
  for epoch=0:param.epochs
      if isMainWorker
        ...
        elseif any(~isfinite(dWB)), tr.stop = 'Precision problems in matrix inversion.'; calcNet = best.net;
        elseif (cons_a0 >= 4), tr.stop = 'Line search resets did not produce a new minimum.'; calcNet = best.net;
        end

        % Feedback
        tr = nnet.trainingRecord.update(tr,[epoch current_time perf vperf tperf gradient val_fail dperf tol delta a cons_a0]);
        statusValues = [epoch,current_time,best.perf,gradient,val_fail,cons_a0];
    ...
    [a,gWB,perf,retcode,delta,tol] = ...
      feval(param.searchFcn,calcLib,calcNet,dWB,gWB,perf,dperf,delta,tol,ch_perf,param);
    ...
    % Validation
    if isMainWorker, ifFlag = (a <= tol) || param.batch_frag; end
    if isParallel, ifFlag = labBroadcast(mainWorkerInd,ifFlag); end
