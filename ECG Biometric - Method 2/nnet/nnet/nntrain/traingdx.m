...
% $Revision: 1.1.6.21 $ $Date: 2013/05/13 23:10:20 $
...
function net = formatNet(net)
  if isempty(net.performFcn)
    warning('nnet:traingdx:Performance',nnwarning.empty_performfcn_corrected);
    net.performFcn = 'mse';
  end
end
...
function [calcNet,tr] = train_network(archNet,rawData,calcLib,calcNet,tr)
  ...
  for epoch=0:param.epochs
    ...
    if isMainWorker
        ...
        tr = nnet.trainingRecord.update(tr,[epoch current_time perf vperf tperf gradient val_fail lr]);
    ...
    if isMainWorker
        dWB = param.mc*dWB + (1-param.mc)*lr*gWB;
        WB2 = WB + dWB;
    end
