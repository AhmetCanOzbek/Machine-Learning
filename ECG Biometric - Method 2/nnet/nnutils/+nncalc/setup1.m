...
% Copyright 2012-13 The MathWorks, Inc.
...
% Gracefully fall back to single process with messages
...
% Must error is SPMD not available as single proces cannot take Composite
...
% If nnGPU or nnGPUOp are requested, check availability
% Gracefully fall back to single process with messages
if nnstring.starts(calcMode.mode,'nnGPU')
  ...
  elseif (~nnGPU.isSupported)
    disp(getStringSafe(message('nnet:gpu:NotAbleToUseParallelPool')));
...
if nnstring.starts(calcMode.mode,'nnMex')
  flag = false;
