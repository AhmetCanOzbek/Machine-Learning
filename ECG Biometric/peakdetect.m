...
hold on,scatter(R_i(1:R(end))./fs,R_amp(1:R(end)),'r','MarkerFaceColor','r');
hold on,scatter(S_i(1:S(end))./fs,S_amp(1:S(end)),'g','MarkerFaceColor','g');
hold on,scatter(T_i(1:T(end))./fs,T_amp(1:T(end)),'k','MarkerFaceColor','k');
hold on,scatter(Q_i(1:Q(end))./fs,Q_amp(1:Q(end)),'m','MarkerFaceColor','m');
...
legend('Raw ECG Signal','R peak','S peak','T peak','Q peak','Latest S wave','T wave adaptive threshold threshold','Location','NorthOutside','Orientation','horizontal'); 
