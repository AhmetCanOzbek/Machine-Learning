...
if __name__ == '__main__':
    ...
    df_input = pandas.read_csv('wiki_parse_u2z.bin', header=None, delimiter = "|", names=col_input)
        ...
        try:
            ...
            if requests.head(url_to_parse).status_code == 200:
                ...
                if file_io_counter % 50 == 0:
                    if counter == 1:
                        my_array_data_extract_wiki_1 = numpy.array([row['track_id'], row['song_id'], page, album_name, extracted_date, genre])
                    else :
                        my_array_data_extract_wiki_1 = numpy.vstack((my_array_data_extract_wiki_1,numpy.array([row['track_id'], row['song_id'], page, album_name, extracted_date, genre])))

                    f_handle = file('MSD_Data_Extract_Wiki_u2z.bin','a')
                    numpy.savetxt(f_handle, my_array_data_extract_wiki_1, delimiter='|',fmt='%s')
                    f_handle.close()

                    del my_array_data_extract_wiki_1
                    counter = 0
                    file_io_counter = 0
                else:
                    if counter == 1:
    ...
    try:
        my_array_data_extract_wiki_u2z
    except NameError:
        print "my_array_data_extract_wiki_1 does not exist currently"
    else:
        f_handle = file('MSD_Data_Extract_Wiki_u2z.bin','a')
        numpy.savetxt(f_handle, my_array_data_extract_wiki_1, delimiter='|',fmt='%s')
        f_handle.close()
