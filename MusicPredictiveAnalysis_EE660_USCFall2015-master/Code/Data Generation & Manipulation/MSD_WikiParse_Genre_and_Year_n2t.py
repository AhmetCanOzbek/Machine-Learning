...
if __name__ == '__main__':
    ...
    df_input = pandas.read_csv('wiki_parse_n2t.bin', header=None, delimiter = "|", names=col_input)
        ...
        try:
            ...
            if requests.head(url_to_parse).status_code == 200:
                ...
                if file_io_counter % 50 == 0:
                    if counter == 1:
                        my_array_data_extract_wiki_n2t = numpy.array([row['track_id'], row['song_id'], page, album_name, extracted_date, genre])
                    else :
                        my_array_data_extract_wiki_n2t = numpy.vstack((my_array_data_extract_wiki_n2t,numpy.array([row['track_id'], row['song_id'], page, album_name, extracted_date, genre])))

                    f_handle = file('MSD_Data_Extract_Wiki_n2t.bin','a')
                    numpy.savetxt(f_handle, my_array_data_extract_wiki_n2t, delimiter='|',fmt='%s')
                    f_handle.close()

                    del my_array_data_extract_wiki_n2t
                    counter = 0
                    file_io_counter = 0
                else:
                    if counter == 1:
    ...
    try:
        my_array_data_extract_wiki_n2t
    except NameError:
        print "my_array_data_extract_wiki_n2t does not exist currently"
    else:
        f_handle = file('MSD_Data_Extract_Wiki_n2t.bin','a')
        numpy.savetxt(f_handle, my_array_data_extract_wiki_n2t, delimiter='|',fmt='%s')
        f_handle.close()
