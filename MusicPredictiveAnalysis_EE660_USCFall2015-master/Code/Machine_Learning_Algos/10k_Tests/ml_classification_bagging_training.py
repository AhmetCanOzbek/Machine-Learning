...
if __name__ == '__main__':
    ...
    # Bagging
    bagging1 = BaggingClassifier(KNeighborsClassifier(n_neighbors=2),max_samples=0.3, max_features=0.1)
    bagging1.fit(df_input1_data,numpy.ravel(df_input1_target))
    pickle.dump(bagging1, open('model_bagging_t1.pkl', 'wb'))

    bagging2 = BaggingClassifier(KNeighborsClassifier(n_neighbors=2),max_samples=0.3, max_features=0.1)
    bagging2.fit(df_input2_data,numpy.ravel(df_input2_target))
    pickle.dump(bagging2, open('model_bagging_t2.pkl', 'wb'))

    bagging3 = BaggingClassifier(KNeighborsClassifier(n_neighbors=2),max_samples=0.3, max_features=0.1)
    ...
    predicted1 = bagging1.predict(x_df_input1_data)
    predicted2 = bagging2.predict(x_df_input2_data)
    predicted3 = bagging3.predict(x_df_input3_data)
    predicted4 = bagging4.predict(x_df_input4_data)
    predicted5 = bagging5.predict(x_df_input5_data)
    # predicted = bagging.predict(x_df_input_data)
